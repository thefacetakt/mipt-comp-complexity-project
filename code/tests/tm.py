...
class TestTM(unittest.TestCase):
    ...
    def test_tm2(self):
        ...
        tm.delta[(1, ord('#'))] = Transition(2, 3, R)
        tm.tape[0] = 1
        count = 0
        while (not tm.make_step() and count < MAX_COUNT):
            count += 1
        ...
        self.assertEquals(tm.tape[2], ord('#'))
        self.assertEquals(tm.tape[-1], ord('#'))
        self.assertEquals(tm.current_position, 2)

    def test_tm3(self):
        tm = TM(2)
        tm.delta[(0, ord('#'))] = Transition(0, 2, L)
